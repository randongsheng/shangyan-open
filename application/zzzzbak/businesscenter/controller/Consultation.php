...
class Consultation extends Common
{
	...
	public function consultationOrder(){
    	...
    	$count = db('order o')->field('orderid,o.createtime,nickname,realname,clinic_name,o.topic,ordermoney,sytime,alltime,o.status')->join('user u','u.id=o.uid')->join('userfield f','f.uid=o.serverpersonid')->join('clinic c','c.id=o.clinicid')->count();
    	$pageSize = 10;
	...
	public function orderInfo(){
		...
		$order = db('order')->field('orderid,ordermoney,topic,clinicid,paymode,uid,content,serverpersonid')->where(['orderid'=>$orderid])->find();
		...
		$visitor = db('user')->field('avatarurl,nickname,id,uname,gender,mobile')->where(['id'=>$order['uid']])->find();
		$teacher = db('user u')->field('avatarurl,level,id,realname,mobile,gender,title')->join('userfield f','u.id=f.uid')->where(['id'=>$order['serverpersonid']])->find();
		$clinic = db('clinic')->field('level,clinic_name,logo,id,nature,operator_tel')->where(['id'=>$order['clinicid']])->find();
		$ordermore = db('ordermore m')->field('m.id,status,date,week,starttime,endtime,content,score,teachersay')->join('zixuncomment z','m.id=z.moreid','LEFT')->where(['orderid'=>$orderid])->select();
		//print_r($ordermore);
