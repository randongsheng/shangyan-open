...
class Userapp extends Controller
{
    ...
    * 
    ...
    public function bootPage(){
    	$list = Db::name('banner')->where(['position'=>3])->order('sort desc')->select();
    ...
    public function getBootPage(){
    	$id = input('post.id');
    	$bootPage = Db::name('banner')->where(['id'=>$id])->find();
    	if ($bootPage) {
    ...
    * 
    ...
    public function getBanner($position){
    	$list = Db::name('banner')->where(['position'=>$position])->order('sort asc')->select();
    ...
    * 
     */
   	...
   	public function getPushClinic(){
   		$list = Db::name('clinic')->field('id,clinic_name,update_at,sort')->where(['push'=>1])->order('sort asc')->select();
   		sendJson(1,'商户列表',$list);
   	}
   	//获取推荐老师
   	public function getPushTeacher(){
   		$list = Db::name('user u')->field('id,realname,gender,sort,last_login_time')->join('userfield f','u.id=f.uid')->where(['push'=>1])->order('sort asc')->select();
   		sendJson(1,'老师列表',$list);
   	}
   	//获取推荐测试
   	public function getPushExam(){
   		$list = Db::name('examtitle')->field('id,title,topic,sort,update_at')->where(['push'=>1])->order('sort asc')->select();
   	...
   	public function getPushArticles(){
   		$list = Db::name('articles')->field('id,title,author,keywords,sort,update_at')->where(['push'=>1])->order('sort asc')->select();
   		foreach ($list as $key => $value) {
   	...
   	public function getPushHotWord(){
   		$list = Db::name('hotword')->field('id,hotword,update_at,sort')->where(['push'=>1])->order('sort asc')->select();
