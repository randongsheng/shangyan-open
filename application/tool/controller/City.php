...
class City extends Controller
{
	public function getCity($city){
		$province = '[{ "name": "beijing", "cities": ["xicheng", "dongcheng", "chongwen", "xuanwu", "chaoyang", "haidian", "fengtai", "shijingshan", "mentougou", "fangshan", "tongzhou", "shunyi", "daxing", "changping", "pinggu", "huairou", "miyun", "yanqing"] },
					...
					$data1 = json_decode($province1,true);
					...
					for ($i=0; $i < $provincecount; $i++) { 
						...
						$citycount = count($data[$i]['cities']);
						for ($u=0; $u < $citycount; $u++) { 
							if ($city == $data[$i]['cities'][$u]) {
								return $data1[$i]['cities'][$u];
							}
						}
					}
	}
